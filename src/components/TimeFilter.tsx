...
export function TimeFilter({ selected, onChange }: TimeFilterProps) {
  ...
  return (
    <div className="inline-flex items-center gap-1 p-1 bg-[#131313] border border-white/10 rounded-xl">
      {options.map((opt) => {
        ...
        return (
          <button
            ...
            className={`flex items-center gap-1.5 px-3.5 py-1.5 text-xs font-mono rounded-lg transition-all duration-150 cursor-pointer ${
              isActive
                ? 'bg-[#16DC76] text-black font-extrabold shadow-[0_0_12px_rgba(22,220,118,0.3)]'
